...
def forward_mapping(input_img, rec_img, markers, n_pertubations, model, debug=True):
    roi = markers != 0
    print(roi.shape)

    max_range = int(normalization_value(input_img))  # e.g., 255 in 8-bit image

    # step = (2 * max_range) / n_pertubations
    # # the step may generate an extra value, so we only get the first `n_pertubations` numbers (float)
    # pertubations = np.arange(-max_range, max_range + 1, step)[:n_pertubations]

    pertubations = np.arange(-n_pertubations, n_pertubations + 1, 2)[:n_pertubations]
    shape = tuple([n_pertubations] + list(input_img.shape))  # (n_pertubations, ysize, xsize)
    ...
    reps = tuple([n_pertubations] + [1] * input_img.ndim)
